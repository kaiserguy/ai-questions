...
class AIWikipediaSearch {
    ...
    async loadDatabaseFromFile(file) {
        ...
        // Copilot #7: Check for specific wikipedia_articles table, not just any table
        const tables = database.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='wikipedia_articles'");
        if (!tables || tables.length === 0 || !tables[0]?.values || tables[0].values.length === 0) {
            throw new Error('Database does not contain a wikipedia_articles table');
        }
